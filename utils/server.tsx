 ...
 * and converts the generator to a RSC friendly streamqqqqaqqqaaa
 *
 * @param runnable
...
export function streamRunnableUI<RunInput, RunOutput>(
  ...
  (async () => {
    // track the final stream event
    let lastEventValue: StreamEvent | null = null;
    // map run_d to streamable values for text streaming 
    const callbacks: Record<
      string,
      ReturnType<typeof createStreamableUI | typeof createStreamableValue>
    > = {};

    for await (const streamEvent of (runnable as Runnable<RunInput, RunOutput>).streamEvents(inputs, {version: "v2", })) 
    {
      if (streamEvent.name === CUSTOM_UI_YIELD_NAME && isValidElement(streamEvent.data.output.value)) 
      {
        if (streamEvent.data.output.type === "append") {
          ui.append(streamEvent.data.output.value); // append UI component
        } else if (streamEvent.data.output.type === "update") {
          ui.update(streamEvent.data.output.value);//update the UI component with the data returned by the tool
        }
      }

      if (streamEvent.event === "on_chat_model_stream") { //text response from llm
